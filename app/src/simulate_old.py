...
async def run_simulation(agent, steps=1):
    ...
    for tick in range(steps):
        ...
        # Enforce schedule slot dominance: no drift before slot end
        slot, slot_start, slot_end = cur_slot, cur_start, cur_end
        if slot and slot_end and current_time < slot_end:
            drift["should_drift"] = False
            drift["drift_type"] = "none"
            drift["drift_topic"] = ""
            drift["drift_intensity"] = 0
            drift["drift_action"] = "continue"
            drift["potential_recovery"] = ""
            drift["justification"] = ""
            action_result["drift_type"] = "none"
            action_result.pop("drift_topic", None)
        ...
        if drift.get("drift_type") == "none":
            act = (
                ...
                or plan.get("action")
                or obs.get("action")
                or "doing planned action"
            )
            loc = (
                action_result.get("location")
                or plan.get("location")
                or obs.get("location")
                or "current location"
            )
            topic = action_result.get("topic") or plan.get("topic") or ""
            topic_clause = f" about {topic}" if topic else ""
            action_result["state_summary"] = plan.get("state_summary") or obs.get(
                "state_summary"
            )
